...
interface HomeProps {
  onNavigate: (tab: 'home' | 'profile' | 'vault' | 'secure-line') => void
}
...
export default function Home({ onNavigate }: HomeProps) {
  ...
  // 1. Boot Sequence Log Injection Simulation
  useEffect(() => {
  ...
  const handleCommandSubmit = (e: React.FormEvent) => {
    ...
    // A. PASSWORD GATE (Terminal-Native Input Listener)
    ...
    setTimeout(() => {
      switch (cleanCmd) {
        ...
        case 'demo':
          ...
          setTimeout(() => onNavigate('vault'), 800)
          break
        ...
        case 'contact':
          ...
          setTimeout(() => onNavigate('secure-line'), 800)
          break
  ...
  return (
    <div className='space-y-8 max-w-2xl font-mono text-xs text-[var(--color-text-main)] animate-scan p-4 relative h-full'>
      {/* FULL-SCREEN AUTH SEQUENCE WITH PASS/FAIL DETERMINATION */}
      {authStage !== 'idle' && (
        <AuthSequence
